...
var bookingInquiries = mysqlTable("bookingInquiries", {
  id: int("id").autoincrement().primaryKey(),
  fullName: varchar("fullName", { length: 120 }).notNull(),
  whatsapp: varchar("whatsapp", { length: 32 }).notNull(),
  email: varchar("email", { length: 320 }),
  campus: varchar("campus", { length: 180 }),
  studyProgram: varchar("studyProgram", { length: 160 }).notNull(),
  thesisStage: varchar("thesisStage", { length: 120 }).notNull(),
...
async function createBookingInquiry(inquiry) {
  const db = await getDb();
  if (!db) throw new Error("Database pemesanan belum tersedia");
  await db.insert(bookingInquiries).values(inquiry);
}
...
var bookingInput = z3.object({
  fullName: z3.string().trim().min(2, "Nama minimal dua karakter").max(120),
  whatsapp: z3.string().trim().min(8, "Nomor WhatsApp belum valid").max(32),
  email: z3.string().trim().email("Email belum valid").max(320).optional().or(z3.literal("")),
  campus: z3.string().trim().max(180).optional().or(z3.literal("")),
  studyProgram: z3.string().trim().min(2, "Program studi wajib diisi").max(160),
  thesisStage: z3.string().trim().min(2, "Pilih tahap skripsimu").max(120),
  topicNote: z3.string().trim().max(2e3).optional().or(z3.literal("")),
  productHandle: z3.string().trim().max(255).optional(),
  productTitle: z3.string().trim().max(255).optional(),
  intent: z3.string().trim().max(120).optional()
});
